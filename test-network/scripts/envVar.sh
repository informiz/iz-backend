...
setOrdererGlobals() {
  ...
  export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/ordererOrganizations/informiz.org/orderers/orderer.informiz.org/msp/tlscacerts/tlsca.informiz.org-cert.pem
  export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/ordererOrganizations/informiz.org/users/Admin@informiz.org/msp
}
...
setGlobals() {
  ...
  if [ $USING_ORG -eq 1 ]; then
    ...
    export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/_ORG_NAME_/users/Admin@_ORG_NAME_/msp
    export CORE_PEER_ADDRESS=localhost:7051
  elif [ $USING_ORG -eq 2 ]; then
    export CORE_PEER_LOCALMSPID="Org2MSP"
    export CORE_PEER_TLS_ROOTCERT_FILE=$PEER0_ORG2_CA
    export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/_FACT_CHECKERS_ORG_/users/Admin@_FACT_CHECKERS_ORG_/msp
