...
export PATH=${PROJECT_DIR}/bin:${PWD}:$PATH
...
function printHelp() {
  ...
  echo "      - 'deployCC' - deploy the informiz chaincode on the channel"
...
function deployCC() {
  ...
  scripts/deployCC.sh $CHANNEL_NAME $VERSION $CLI_DELAY $MAX_RETRY $VERBOSE
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts log.txt informiz.tar.gz informiz
